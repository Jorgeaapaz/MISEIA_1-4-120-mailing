...
export async function POST(_req: NextRequest, ctx: RouteContext<'/api/plantillas/[id]/preview'>) {
  const req = _req
  ...
  try {
    ...
    return Response.json({ error: `Error al renderizar: ${(err as Error).message}` }, { status: 400 })
  }
}
